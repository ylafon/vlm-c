...
# $Id: vlmc-shm-unittest.php,v 1.1 2008/08/11 15:17:10 ylafon Exp $
#
...
<html>
  ...
  <body>
  <h1>Unit test: distance from Point 1 to Line A-B</h1>
  <pre>
<?php
...
?>
  ...
  <h1>Unit test: distance from Point 2 to Line A-B</h1>
  <pre>
<?php
...
for($i =0; $i < 40 ; $i++ ) {
    ...
    # each 15mn
    get_wind_info_latlong_deg_UV($lat_boat, $long_boat, $current_time + (5 * 60 * $i), $wind_boat);
    printf("UV   Wind at  lat: %.2f long: %.2f, speed %.1f angle %.1f\n", $lat_boat,
    		  $long_boat, $wind_boat->speed, $wind_boat->angle);
    get_wind_info_latlong_deg_TWSA($lat_boat, $long_boat, $current_time + (15 * 60 * $i), $wind_boat);
